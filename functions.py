...
datas = readSetting()
userdata= ["ipb_memeber_id=" +datas["ipb_member_id"] , "ipb_pass_hash="+datas["ipb_pass_hash"],"ipb_session_id="+datas["ipb_session_id"],"sk="+datas["sk"]]
cookies =""
for i in userdata:
    cookies+=i+"; "
...
def download_image(url: str):
    ...
    # Create the folder
    if os.path.isdir(GN) == False:os.mkdir(GN)
    downloaded = False
    while not downloaded:
        try:
...
def checkIMGlimit():
    ...
    while int(limit)>=4950:
        ...
        print()
        print(limittext+(get_terminal_size().columns-len(limittext)-8)*" "+ctime()[11:-5],end="\r")
        sleep(20)
        home = download("https://e-hentai.org/home.php")
        bs = BeautifulSoup(home,"html.parser")
        limit = bs.find("strong").contents[0]
